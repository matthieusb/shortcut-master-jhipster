...
public class ShortcutResourceIntTest {
    ...
    @Mock
    private ShortcutRepository shortcutRepositoryMock;
    
    @Mock
    private ShortcutService shortcutServiceMock;
    ...
    public void getAllShortcutsWithEagerRelationshipsIsEnabled() throws Exception {
        ShortcutResource shortcutResource = new ShortcutResource(shortcutServiceMock);
        when(shortcutServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restShortcutMockMvc = MockMvcBuilders.standaloneSetup(shortcutResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restShortcutMockMvc.perform(get("/api/shortcuts?eagerload=true"))
        .andExpect(status().isOk());

        verify(shortcutServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    public void getAllShortcutsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ShortcutResource shortcutResource = new ShortcutResource(shortcutServiceMock);
            when(shortcutServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restShortcutMockMvc = MockMvcBuilders.standaloneSetup(shortcutResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
    ...
    public void updateShortcut() throws Exception {
        ...
        // Update the shortcut
        Shortcut updatedShortcut = shortcutRepository.findById(shortcut.getId()).get();
